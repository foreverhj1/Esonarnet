...
import numpy as np
import torch
from nets.model_training import CE_Loss, Dice_loss, Focal_Loss,Tversky_loss
...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        if iteration >= epoch_step: 
            break
        imgs, hog_imgs, pngs, labels = batch
        # print(hog_imgs.shape)
        # test_img = imgs[0, :, :, :].numpy() * 255
        # test_hog_img = hog_imgs[0, :, :, :].numpy() * 255
        # cv2.imwrite('imgs.jpg', np.transpose(test_img.astype(np.uint8), (1, 2, 0)))
        # cv2.imwrite('hog_imgs.jpg', np.transpose(test_hog_img.astype(np.uint8), (1, 2, 0)))
        with torch.no_grad():
            weights = torch.from_numpy(cls_weights)
            if cuda:
                imgs    = imgs.cuda(local_rank)
                hog_imgs = hog_imgs.cuda(local_rank)
        ...
        if not fp16:
            ...
            #   前向传播
            #----------------------#
            outputs = model_train(imgs, hog_imgs)
            #----------------------#
            #   损失计算
            #----------------------#
            if focal_loss:
            ...
            if dice_loss:
                tversky_loss = Tversky_loss(outputs, labels, a=0.5, b=0.5)
                main_dice = Dice_loss(outputs, labels)
                #loss      = loss + main_dice+tversky_loss
                loss = loss + main_dice
        ...
        if not fp16:
            ...
            with autocast():
                ...
                if dice_loss:
                    main_dice = Dice_loss(outputs, labels)
                    tversky_loss = Tversky_loss(outputs, labels, a=0.5, b=0.5)
                    #oss      = loss + main_dice+tversky_loss
                    loss = loss + main_dice
