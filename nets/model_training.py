...
def CE_Loss(inputs, target, cls_weights, num_classes=21):
    ...
    temp_inputs = inputs.transpose(1, 2).transpose(2,
                                                   3).contiguous().view(-1, c)
    temp_target = target.view(-1)

    CE_loss = nn.CrossEntropyLoss(weight=cls_weights,
                                  ignore_index=num_classes)(temp_inputs,
                                                            temp_target)
    return CE_loss
...
def Focal_Loss(inputs,
    ...
    temp_inputs = inputs.transpose(1, 2).transpose(2,
                                                   3).contiguous().view(-1, c)
    temp_target = target.view(-1)
    #print('1111111111111',temp_inputs.shape,temp_target.shape)

    logpt = -nn.CrossEntropyLoss(weight=cls_weights,
...
def Tversky_loss(inputs, target, a=0.5, b=0.5):
    n, c, h, w = inputs.size()
    nt, ht, wt, ct = target.size()
    if h != ht and w != wt:
        inputs = F.interpolate(inputs,
    ...
    ad = torch.sum(temp_target[..., :-1] + temp_inputs, axis=[0, 1])
    tp = torch.sum(temp_target[..., :-1] * temp_inputs, axis=[0, 1])
    fp = torch.sum(temp_inputs, axis=[0, 1]) - tp
    fn = torch.sum(temp_target[..., :-1], axis=[0, 1]) - tp

    score = (tp + 1) / (tp + a * fp + b * fn + 1)
    tversky_loss = 1 - torch.mean(score)

    score1 = tp / (ad - tp)
    iou_loss = 1 - torch.mean(score1)
