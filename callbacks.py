...
class EvalCallback():
    ...
    def get_miou_png(self, image, hog_image):
        ...
        hog_image, nw, nh = resize_image(hog_image, (self.input_shape[1], self.input_shape[0]))
        #---------------------------------------------------------#
        #   添加上batch_size维度
        #---------------------------------------------------------#
        image_data  = np.expand_dims(np.transpose(preprocess_input(np.array(image_data, np.float32)), (2, 0, 1)), 0)
        hog_image = np.expand_dims(np.transpose(preprocess_input(np.array(hog_image, np.float32)), (2, 0, 1)), 0)
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                hog_image_path = os.path.join(self.dataset_path, "VOC2007/HOG/" + image_id + ".png")
